...
<form method="POST" action="{{ route('sessions.store') }}" class="test-layout" id="session-form">
    ...
    <section class="panel">
        ...
        <div class="profile-grid">
            ...
            <button type="button" class="profile-card active" data-profile="balanced"><strong>Balanced</strong><span>Recommended coverage termasuk sensitive files, authenticated access, account defense, dan Laravel manifest.</span></button>
            <button type="button" class="profile-card" data-profile="deep"><strong>Deep Safe</strong><span>Semua modul termasuk source-assisted validation dan controlled resilience.</span></button>
        </div>
            ...
            @foreach($moduleOptions as $key => [$title, $description])
                <label class="module-card">
                    <input type="checkbox" name="modules[]" value="{{ $key }}" data-module="{{ $key }}"
                        @checked(in_array($key, old('modules', ['headers','tls','cookies','cors','exposure','rate_limit','latency','security_txt','http_methods','dns_posture','sensitive_files','authenticated_access','account_compromise','laravel_agent']), true))>
        ...
        <div class="risk-callout">
            <strong>Authenticated Access, Account Compromise Defense & Laravel Agent</strong>
            <p>Setelah session dibuat, Anda dapat menambahkan akun uji seperti Student, Teacher, Staff, Admin Test, atau Bearer API Client lalu memvalidasi privilege boundary dan IDOR/BOLA.</p>
            <p>Account Security Lab melakukan bounded enumeration/throttling checks pada <b>dedicated test account</b>, tanpa brute-force atau password guessing. Laravel Agent menambahkan route/middleware/config inventory agar endpoint admin/public dan mutating/public dapat ditemukan dari metadata aplikasi.</p>
        </div>
    </section>
...
(() => {
    const presets = {
        ...
        balanced: ['headers','tls','cookies','cors','exposure','rate_limit','latency','security_txt','http_methods','dns_posture','sensitive_files','authenticated_access','account_compromise','laravel_agent'],
        deep: ['headers','tls','cookies','cors','exposure','rate_limit','latency','security_txt','http_methods','dns_posture','sensitive_files','authenticated_access','account_compromise','laravel_agent','load_resilience'],
    };
