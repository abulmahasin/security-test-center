...
@php
    $intervalValue = 24;
    $intervalUnit = 'hours';
    $shouldPoll = in_array($session->status, ['queued', 'running'], true);
...
<section class="session-hero">
    <div>
        <div class="hero-badges">
            ...
            @else
                <span class="status draft">Unverified</span>
            @endif

            @if($session->monitoring_enabled)
                <span class="status completed">Monitoring ON</span>
            @else
                <span class="pill">Monitoring OFF</span>
            @endif
        </div>

        <h2>{{ $session->target_url }}</h2>
        <p>
            {{ count($session->selected_modules ?? []) }} security modules
            • Profile {{ ucfirst($session->profile) }}
            @if($session->monitoring_enabled)
                • {{ $session->monitoringLabel() }}
            @endif
            @if($session->next_run_at)
                • Next {{ $session->next_run_at->format('d M Y H:i') }}
            @endif
        </p>
    </div>
...
<div class="monitoring-panel">
        <div class="panel-head">
            ...
            <span class="status {{ $session->monitoring_enabled ? 'completed' : 'draft' }}">
                {{ $session->monitoring_enabled ? 'ACTIVE' : 'OFF' }}
            </span>
        </div>
        ...
        <form method="POST" action="{{ route('sessions.monitoring', $session) }}" class="monitoring-form">
            ...
            <label class="monitoring-switch">
                <input
                    ...
                    value="1"
                    id="session-monitoring-enabled"
                    @checked($session->monitoring_enabled)
                >
                <span>
                    <strong>Enable Auto Monitoring</strong>
                    <small>Uncheck lalu Save untuk mematikan monitoring sepenuhnya.</small>
                </span>
            </label>
            ...
            <div class="form-grid three monitoring-inline">
                <label>
                    <span>Run Every</span>
                    <input type="number" min="1" max="8760" name="monitoring_interval_value" value="{{ $intervalValue }}">
                </label>

                <label>
                    <span>Unit</span>
                    <select name="monitoring_interval_unit">
                        <option value="hours" @selected($intervalUnit === 'hours')>Hour(s)</option>
                        <option value="days" @selected($intervalUnit === 'days')>Day(s)</option>
                        <option value="weeks" @selected($intervalUnit === 'weeks')>Week(s)</option>
                    </select>
                </label>

                <div class="monitoring-save">
                    <button type="submit" class="btn btn-secondary">Save Monitoring</button>
                </div>
            </div>

            <small class="muted">
                Minimal 1 jam, maksimal 1 tahun.
                @if($session->next_run_at)
                    Next automatic assessment: {{ $session->next_run_at->format('d M Y H:i') }}.
                @endif
            </small>
        </form>
    </section>
    ...
    <section class="run-grid">
        <article class="panel progress-panel">
            ...
            <div class="progress-track">
                <div id="progress-bar" style="width: {{ $session->progress }}%"></div>
            </div>
            ...
            <form method="POST" action="{{ route('sessions.run', $session) }}">
                ...
                <button
                    ...
                    type="submit"
                    @disabled(in_array($session->status, ['queued', 'running'], true))
                >
                    {{ $session->status === 'completed' ? 'Run New Assessment' : 'Start Security Audit' }}
                </button>
            </form>
        </article>
...
@if($session->status === 'completed')
    ...
    <section class="metrics posture-metrics">
        <article class="metric-card">
            ...
            <strong>
                ...
                @if($session->compliance_score !== null)
                    <small class="metric-suffix">%</small>
                @endif
            </strong>
            <small>Selected controls without high-risk failure</small>
        </article>

        <article class="metric-card">
            <span>Score Delta</span>
            <strong class="{{ ($session->risk_delta ?? 0) < 0 ? 'text-danger' : 'text-good' }}">
                {{ $session->risk_delta === null ? '—' : (($session->risk_delta > 0 ? '+' : '').$session->risk_delta) }}
            </strong>
            <small>
                @if($session->baseline)
                    vs baseline #{{ $session->baseline->id }} ({{ $session->baseline->score }})
                @else
                    First baseline
                @endif
            </small>
        ...
        <article class="metric-card">
            ...
            <strong>{{ $session->new_findings_count }}</strong>
            <small>Non-info findings not seen in baseline</small>
        </article>

        <article class="metric-card">
            <span>Resolved</span>
            <strong class="text-good">{{ $session->resolved_findings_count }}</strong>
            <small>Baseline fingerprints no longer detected</small>
        </article>
    </section>
    ...
    <section class="panel">
        <div class="panel-head">
            <div>
                <p class="eyebrow">Continuous Posture Report</p>
                <h2>Risk, Evidence & Solution</h2>
                <p class="muted">Setiap finding menjelaskan dampak praktis dan solusi. Untuk sensitive-file/authenticated/account-security scan, secret dan response body tidak pernah disimpan di report.</p>
            </div>
            ...
            @forelse($session->findings as $finding)
                <article class="finding {{ $finding->change_type === 'new' && $finding->severity !== 'info' ? 'finding-new' : '' }}">
                    ...
                    <div class="remediation">
                        <strong>Solusi / Recommended Remediation</strong>
        ...
        (() => {
            ...
            async function poll() {
                ...
                try {
                    ...
                    if (terminal && Array.isArray(data.logs) && data.logs.length) {
                        terminal.innerHTML = data.logs
                            .map(log => `<div><span>[${new Date(log.created_at).toLocaleTimeString()}]</span> ${escapeHtml(log.message)}</div>`)
                            .join('');
                    ...
                    // Keep polling. A transient connection error should not break the page.
                }
            }
